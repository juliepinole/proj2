...
# from nk import epochs_create, epochs_to_df
# from nk.signal import (
#     signal_findpeaks,
#     signal_formatpeaks,
#     signal_rate,
#     signal_resample,
...
# )
# from nk.stats import standardize
# from nk.ecg_peaks import ecg_peaks
# from .ecg_segment import ecg_segment


def ecg_delineate_custom(
    ecg_cleaned,
    rpeaks=None,
    sampling_rate=1000,
    method="dwt",
    ...
    * Step 2. Plot P-Peaks and T-Peaks
    ...
    if isinstance(ecg_cleaned, pd.DataFrame):
        ...
        for i in ecg_cleaned:
            ...
            if cols:
                signals = nk.epochs_to_df(ecg_cleaned)
    ...
    instant_peaks = nk.signal_formatpeaks(waves_noNA, desired_length=len(ecg_cleaned))
...
def _dwt_adjust_parameters(rpeaks, sampling_rate, duration=None, target=None):
    average_rate = np.median(nk.signal_rate(peaks=rpeaks, sampling_rate=sampling_rate))
    print(nk.signal_rate(peaks=rpeaks, sampling_rate=sampling_rate))
    ...
    if target == "degree":
        ...
        print(f'scale_factor: {scale_factor}, average_rate: {average_rate}, sampling_rate: {sampling_rate}')
...
def _dwt_delineate_tp_peaks(
    ...
    for rpeak_ in rpeaks:
        ...
        for idx_peak, idx_peak_nxt in zip(peaks[:-1], peaks[1:]):
            ...
            if correct_sign:
                idx_zero = (
                    nk.signal_zerocrossings(dwt_local[idx_peak : idx_peak_nxt + 1])[0]
                    + idx_peak
                )
...
def _find_tppeaks(ecg, keep_tp, sampling_rate=1000):
    ...
    for index_cur, index_next in zip(keep_tp[:-1], keep_tp[1:]):
        ...
        if correct_sign:
            index_zero_cr = (
                nk.signal_zerocrossings(cwtmatr[4, :][index_cur : index_next + 1])[0]
                + index_cur
            )
...
def _ecg_delineator_peak_P_onset(rpeak, heartbeat, R, P):
    ...
    try:
        signal = nk.signal_smooth(segment["Signal"].values, size=R / 10)
    except TypeError:
...
def _calculate_abs_z(df, columns):
    ...
    for column in columns:
        df["Dist_R_" + column] = np.abs(
            nk.standardize(df[column].sub(df["ECG_R_Peaks"], axis=0))
        )
    return df
